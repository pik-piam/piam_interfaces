...
for (template in names(templateNames())) {
  test_that(paste0("checkVariableNames in template ", template), {
    t <- getTemplate(template)
    expect_no_warning(checkVariableNames(paste0(t$variable, " (", t$unit, ")")))
    tpiam <- dplyr::filter(t, ! is.na(.data$piam_variable), .data$piam_variable != "TODO")
    expect_no_warning(checkVariableNames(paste0(tpiam$piam_variable, " (", tpiam$piam_unit, ")")))
  })
}
